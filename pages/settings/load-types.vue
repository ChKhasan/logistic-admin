<template>
  <div class="">
    <TitleBlock title="Тип груза" :breadbrumb="['Настройки сайта']" lastLink="Тип груза">
      <div class="d-flex">
        <a-button
  ...
  methods: {
    ...
    deleteAction(id) {
      this.__DELETE_GLOBAL(
        id,
        "fetchLoadTypes/deleteLoadTypes",
        "Успешно удален",
        "__GET_REGIONS"
      );
    },
   ...
   try {
     this.loading = true;
     const data = await this.$store.dispatch("fetchLoadTypes/getLoadTypes", {
       ...this.$route.query,
     });
    ...
    async __POST_REGIONS(data) {
      try {
        await this.$store.dispatch("fetchLoadTypes/postLoadTypes", data);
    ...
    async __GET_REGIONS_BY_ID(targetId) {
      try {
        const data = await this.$store.dispatch("fetchLoadTypes/getLoadTypesById", targetId);
        this.visible = true;
    ...
    async __EDIT_REGIONS(res) {
      try {
        await this.$store.dispatch("fetchLoadTypes/editLoadTypes", {
          id: this.editId,
